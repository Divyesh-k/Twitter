...
const Post = ({ post }) => {
  ...
  const { mutate: likedPost, isPending: isLiking } = useMutation({
    ...
    onSuccess: (updatedLikes) => {
      ...
      // this is not good as it fetch all the  post which is not good.
      // queryClient.invalidateQueries({ querykey: ["posts"] });
  ...
  const { mutate: savePost, isPending: isSaving } = useMutation({
    mutationFn: async () => {
      try {
        const response = await fetch(`/api/posts/addsavedpost/${post._id}`, {
          method: "POST",
        });
        const data = await response.json();
        if (!response.ok) {
    ...
    onSuccess: (updatedSavedPosts) => {
      toast.success(
        updatedSavedPosts.includes(post._id)
          ? "Post Saved Successfully"
          : "Post Removed from Saved Posts"
      );
      queryClient.setQueryData(["authUser"], (oldData) => {
        return { ...oldData, savedPost: updatedSavedPosts };
      });
    },
    onError: (error) => {
      toast.error(error.message);
  ...
  const isSaved = authUser.savedPost.includes(post._id);
        ...
        <div className="flex flex-col flex-1">
          ...
          <div className="flex justify-between mt-3">
            ...
            >
              ...
              {!isSaved && !isSaving && (
                <FaRegBookmark className="w-4 h-4 cursor-pointer text-slate-500 group-hover:text-orange-500" />
              )}
              {isSaved && !isSaving && (
                <FaRegBookmark className="w-4 h-4 cursor-pointer text-orange-500" />
              )}
            </div>
          </div>
        </div>
